...
Usage:
    mri_vp.py <config_file> <run_suffix>
"""
...
if not pathlib.Path(restart_state_dir).exists():
    ...
    # Ay['g'] = file['tasks/A'][minseed_index, :, :, :][slices]
    # Az['g'] = file['tasks/B'][minseed_index, :, :, :][slices]
    # Ax['g'] = file['tasks/C'][minseed_index, :, :, :][slices]
    # Ayx['g'] = file['tasks/Az'][minseed_index, :, :, :][slices]
    # Azx['g'] = file['tasks/Bz'][minseed_index, :, :, :][slices]
    # Axx['g'] = file['tasks/Cz'][minseed_index, :, :, :][slices]

    by = domain.new_field() 
    bz = domain.new_field() 
    bx = domain.new_field() 

    by['g'] = file['tasks/A'][minseed_index, :, :, :][slices]
    bz['g'] = file['tasks/B'][minseed_index, :, :, :][slices]
    bx['g'] = file['tasks/C'][minseed_index, :, :, :][slices]
    Ay['g'], Az['g'], Ax['g'] = vp_bvp_func(domain, by, bz, bx)
   
    vy['g'] = file['tasks/u'][minseed_index, :, :, :][slices]
    vz['g'] = file['tasks/v'][minseed_index, :, :, :][slices]
    vx['g'] = file['tasks/w'][minseed_index, :, :, :][slices]
    p['g'] = file['tasks/p'][minseed_index, :, :, :][slices]
    phi['g'] = file['tasks/Pi'][minseed_index, :, :, :][slices]
...
checkpoints = solver.evaluator.add_file_handler('{}/checkpoints'.format(run_suffix), sim_dt=1.0, max_writes=10, mode=fh_mode)
checkpoints.add_system(solver.state)

slicepoints = solver.evaluator.add_file_handler('{}/slicepoints'.format(run_suffix), sim_dt=0.1, max_writes=50, mode=fh_mode)
