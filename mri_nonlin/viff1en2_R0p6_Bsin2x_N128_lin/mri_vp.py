...
problem.add_equation("Dt(vx) -     f*vy + dx(p) + ν*(dy(ωz) - dz(ωy)) = 0")
problem.add_equation("Dt(vy) + (f+S)*vx + dy(p) + ν*(dz(ωx) - dx(ωz)) = 0")
problem.add_equation("Dt(vz)            + dz(p) + ν*(dx(ωy) - dy(ωx)) = 0")
...
problem.add_equation("dt(Ax) - η * (L(Ax) + dx(Axx)) - dx(phi) - (S*x*bz) = 0")
problem.add_equation("dt(Ay) - η * (L(Ay) + dx(Ayx)) - dy(phi) = 0")
