...
SUFF="viff2en3_R0p6_Bsin2x_N256_tau1e1_U01e0_test2"
MPIPROC=512

mkdir $SUFF
cp $file $SUFF
cp mri_vp.py $SUFF
cp mri_options.cfg $SUFF
cp publication_settings.py $SUFF
cp plot_kebe.py $SUFF
cp plot_ke.py $SUFF
cp plot_be.py $SUFF
cp plot_slicepoints_xy.py $SUFF
cp plot_slicepoints_xz.py $SUFF
cp plot_slicepoints_yz.py $SUFF
cp plot_vb_profiles.py $SUFF
cd $SUFF
...
mpiexec_mpt -np $MPIPROC python3 plot_vb_profiles.py slicepoints_${SUFF}/*.h5 --output=vb_profiles_${SUFF}
...
png2mp4 vp_profiles_${SUFF}/ vp_profiles_${SUFF}.mp4 60
