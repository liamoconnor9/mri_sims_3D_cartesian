...
if __name__ == "__main__":
    ...
    if (write_data):
        ...
        ke_data = {'ke_ar' : [], 'ke_tot_ar' : [], 'sim_times_ar' : []}
        pickle.dump(ke_data, open(path + '/ke_data_' + write_suffix + '.pick', 'wb'))
    ...
    if (plot):
        ...
        shave_ind = 10
        domain_size = np.pi**3 * 8 * 8
        plt.plot(sim_times_ar[:-shave_ind], ke_ar[:-shave_ind] / domain_size, color='darkblue', label=r'$\langle |\mathbf{u}|^2 \rangle_{\mathcal{D}}$')
        plt.plot(sim_times_ar[:-shave_ind], ke_tot_ar[:-shave_ind] / domain_size, color='darkred', label=r'$\langle |\mathbf{u}|^2 \rangle_{\mathcal{D}}$ + Sx')
        plt.yscale('log')
        if (regress):
