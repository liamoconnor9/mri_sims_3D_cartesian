...
if restart_hires:
    ...
    hires_factor = 1
hardwall = False
filename = Path('mri_options_diff1en3.cfg')
outbase = Path("data")
...
problem.substitutions['vector_dot_grad(Ax, Ay, Az, C)'] = "Ax*dx(C) + Ay*dy(C) + Az*dz(C)"
...
problem.add_equation("Dt(jx) - B*dz(ωx) + S*dz(bx) - η*( dx(jxx) + L(jx) ) = b_dot_grad(ωx) - v_dot_grad(jx)"
 + "(dy(bx)*dx(vz) + dy(by)*dy(vz) + dy(bz)*dz(vz)) - (dz(bx)*dx(vy) + dz(by)*dy(vy) + dz(bz)*dz(vy))"
 + "- (dy(vx)*dx(bz) + dy(vy)*dy(bz) + dy(vz)*dz(bz)) + (dz(vx)*dx(by) + dz(vy)*dy(by) + dz(vz)*dz(by))", condition='(ny != 0) or (nz != 0)')

problem.add_equation("Dt(jx) - B*dz(ωx) + S*dz(bx) - η*( dx(jxx) + L(jx) ) = b_dot_grad(ωx) - v_dot_grad(jx)"
 + "vector_dot_grad(dy(bx), dy(by), dy(bz), vz) - (dz(bx)*dx(vy) + dz(by)*dy(vy) + dz(bz)*dz(vy))"
 + "- (dy(vx)*dx(bz) + dy(vy)*dy(bz) + dy(vz)*dz(bz)) + (dz(vx)*dx(by) + dz(vy)*dy(by) + dz(vz)*dz(by))", condition='(ny != 0) or (nz != 0)')
