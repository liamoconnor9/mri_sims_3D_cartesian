...
# 3D MRI
problem_variables = ['p', 'vx','vy','vz','Ax','Ay','Az','Axx','Ayx','Azx', 'phi', 'ωy','ωz', 'B', 'Bx']
...
B.meta['y', 'z']['constant'] = True

# print(np.shape(domain.grid(2)))
# sys.exit()
...
problem.add_equation("Dt(vx) -     f*vy + dx(p) + ν*(dy(ωz) - dz(ωy)) = b_dot_grad(bx) - v_dot_grad(vx) + B*dz(bx)")
...
problem.add_equation("dt(B) - η*dx(Bx) = 0")
...
problem.add_equation("left(B) = 0")
problem.add_equation("right(B) = 0")
