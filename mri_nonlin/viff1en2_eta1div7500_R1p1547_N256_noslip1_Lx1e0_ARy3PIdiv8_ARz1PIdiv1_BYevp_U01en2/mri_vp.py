...
if not pathlib.Path(restart_state_dir).exists():
    ...
    # rand2 = np.random.RandomState(seed=24 + CW.rank)
    ...
    # u, s, vh = np.linalg.svd(by_vec, full_matrices=False)
    Ay['g'] += noise / 10000.0

    # logger.info(np.max(np.abs(Ay['g'])))
    # logger.info(np.max(np.abs(Az['g'])))
    # logger.info(np.max(np.abs(Ax['g'])))
    # logger.info(np.max(phi['g']))
