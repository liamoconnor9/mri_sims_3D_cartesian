"""
Plot kinetic energy from joint analysis files.

Usage:
    plot_ke.py <files>... [--dir=<dir>, --config=<config_file>, --suffix=<run_suffix>]
...
def main(filename, start, count, output):
    ...
    with h5py.File(filename, mode='r') as file:
        ...
        ke_ary = ke_data['ke_ary']
        ke_arz = ke_data['ke_arz']
        # ke_max_ar = ke_data['ke_max_ar']
        sim_times_ar = ke_data['sim_times_ar']
        for index in range(start, start+count):
            ke_avgx = file['tasks']['ke_x'][index][0, 0, 0]
            ke_avgy = file['tasks']['ke_y'][index][0, 0, 0]
            ke_avgz = file['tasks']['ke_z'][index][0, 0, 0]
            ke_arx.append(ke_avgx)
            ke_ary.append(ke_avgy)
            ke_arz.append(ke_avgz)
            sim_times_ar.append(file['scales/sim_time'][index])
        ke_data['ke_arx'] = ke_arx
        ke_data['ke_ary'] = ke_ary
        ke_data['ke_arz'] = ke_arz
...
if __name__ == "__main__":
    ...
    if (write_data):
        ...
        ke_data = {'ke_arx' : [], 'ke_ary' : [], 'ke_arz' : [], 'sim_times_ar' : []}
        pickle.dump(ke_data, open(path + '/ke_data_' + write_suffix + '.pick', 'wb'))
    ...
    if (plot):
        ...
        shave_ind = 10
        plt.plot(sim_times_ar[:-shave_ind], np.array(ke_arx[:-shave_ind]), color='darkblue', label=r'$\langle u^2_x \rangle_{\mathcal{D}}$')
        plt.plot(sim_times_ar[:-shave_ind], np.array(ke_ary[:-shave_ind]), color='darkred', label=r'$\langle u^2_y \rangle_{\mathcal{D}}$')
        plt.plot(sim_times_ar[:-shave_ind], np.array(ke_arz[:-shave_ind]), color='darkgreen', label=r'$\langle u^2_z \rangle_{\mathcal{D}}$')
        plt.yscale('log')
        if (regress):
        ...
        plt.savefig(dir + write_suffix + '/ke_nonlin_' + write_suffix + '.png')
