...
def build_problem(domain, coords, Reynolds):
    ...
    # problem.add_equation("dt(u_t) + grad(p_t) + nu*lap(u_t) = (dot(u_t, (grad(u))) - dot(u, grad(u_t))) + s_t*grad(s)")
    problem.add_equation("dt(u_t) + grad(p_t) + nu*lap(u_t) = (dot(u_t, transpose(grad(u))) - dot(u, grad(u_t))) + s_t*grad(s)")
