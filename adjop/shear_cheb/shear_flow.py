...
# problem.add_equation("u @ grad_psi = 0")
...
flow.add_property(disc, name='disc')
...
try:
    ...
    while solver.proceed:
        ...
        if (solver.iteration-1) % 10 == 0:
            ...
            max_disc = np.sqrt(flow.max('disc'))
            logger.info('Iteration=%i, Time=%e, dt=%e, max(w)=%f, disc=%f' %(solver.iteration, solver.sim_time, timestep, max_w, max_disc))
    solver.step(timestep)
except:
