...
We define a generalized solution state vector $\vec{U}$ and associated initial value problem
\begin{align}
  \mathcal{F}[\vec{U}] &\equiv \partial_t \vec{U} + L[\vec{U}] + N[\vec{U}, \vec{U}] = 0 \label{genpde}
  \intertext{Given a target end state $\vec{U}(\vec{x}, T)$, our aim is to recover the associated initial condition $\vec{U}(x, 0)$.  
  This is realized by performing gradient-based minimization of the functional}
  H[\vec{u}, \vec{U}] &= \frac{1}{2}\davg{|\vec{u}(\vec{x}, T) - \vec{U}(\vec{x}, T)|^2} = \frac{1}{2}\davg{\vec{u}'^2(\vec{x}, T)} \label{Hdef}, 
\end{align}
where the operator $\davg{\cdot}$ denotes the domain-integrated average and the quantity $\vec{u}'(\vec{x}, T)$ refers to $\vec{u}(\vec{x}, T)$'s the deviation from the target solution. For the remainder of this paper, capital symbols (such as $\vec{u}(\vec{x}, T)$) will be used to denote the target solution, while their corresponding lowercase symbols (such as $\vec{u}(\vec{x}, T)$) play the role of optimization parameters.

The Lagrangian associated with our initial value problem is given by
\begin{align*}
  \mathcal{L} &= \int_0^T \davg{\vec{\mu} \cdot \mathcal{F}[\vec{u}]} dt + H[\vec{u}, \vec{U}].
  \intertext{Where $\vec{\mu}(\vec{x}, T)$ is the Lagrangian Multiplier corresponding to $\vec{u}(\vec{x}, T)$. Equating} \frac{\delta\L}{\delta \vec{u}} &= 0 \quad \text{and} \quad \frac{\delta\L}{\delta \vec{\mu}} = 0 
  \intertext{yields the original and adjoint systems} 
  \mathcal{F}[\vec{u}] &= 0 \quad \text{and} \quad \mathcal{F}^{\dagger}[\vec{\mu}, u] = 0 \intertext{respectively. At some particular guess $\vec{u}^n(\vec{x}, 0)$, we compute the functional derivative}
  \frac{\delta\L}{\delta \vec{u}^n(\vec{x}, 0)} &= -\vec{\mu}^n(\vec{x}, 0)
  \intertext{to determine the direction of steepest descent of our objective functional $H[\vec{u}, \vec{U}]$ with respect to the optimization parameter $\vec{u}(\vec{x}, 0)$. Naive gradient-descent can then be performed by computing an updated guess}
  \vec{u}^{n+1}(\vec{x}, 0) &= \vec{u}^n(\vec{x}, 0) + \varepsilon \vec{\mu}^n(\vec{x}, 0)
...
In the context of our time-inversion optimization problem, such techniques traverse the space of possible initial conditions $\vec{u}(\vec{x}, 0)$ by relying on local information pertaining to the objective functional $H$ and its functional derivative $\frac{\delta H}{\delta \vec{u}(\vec{x}, 0)}$. 
...
In doing so, we highlight the challenges and deficits associated with using the least squares functional to recover the initial condition $\vec{U}(\vec{x}, 0)$ belonging to a target end state $\vec{U}(\vec{x}, T)$.
