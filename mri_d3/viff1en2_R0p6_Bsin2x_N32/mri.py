...
problem.add_equation("dt(u) + dot(U0, grad_u) + dot(u, grad(U0)) - nu*div(grad_u) + grad(p) + lift(tau2u,-1) = cross(curl(b), b) - dot(u,grad(u)) - cross(fz_hat, u)")
...
# u['g'][0] = x * (Lx - x)*(randx*randy*randz) / 1e10
# u['g'][1] = x * (Lx - x)
...
slicepoints = solver.evaluator.add_file_handler('slicepoints_' + run_suffix, iter = 1, max_writes=10)
